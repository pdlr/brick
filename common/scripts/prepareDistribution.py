...
BRICK_COMMON = os.path.realpath(os.curdir)
GREP = '/bin/grep'
MAKE = '/usr/bin/make'
MV = '/bin/mv'
RM = '/bin/rm'
...
if __name__ == '__main__':
  ...
  errorNumber += 1
  if os.system(TAR + ' -zxvf %s/brickcommon-"%s".tar.gz' % (BRICK_COMMON, VERSION)) != 0:
    sys.exit(errorNumber)
  # end if

  os.chdir('brickcommon-%s' % VERSION)
  ...
  errorNumber += 1
  if os.system(TAR + ' -zcvf %s/brickcommon-"%s"_htmlDoc.tgz html'
               % (BRICK_COMMON, VERSION)) != 0:
    sys.exit(errorNumber)
  # end if
  ...
  if 0:
    ...
    errorNumber += 1
    if os.system(MV + ' refman.pdf %s/brickcommon-"%s"_manual.pdf'
                 % (BRICK_COMMON, VERSION)) != 0:
      sys.exit(errorNumber)
    # end if
  #end if
  ...
  errorNumber += 1
  if os.system(RM + ' -rf brickcommon-"%s"' % VERSION) != 0:
    sys.exit(errorNumber)
  # end if
