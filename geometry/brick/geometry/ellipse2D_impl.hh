  ...
  namespace geometry {
    ...
    estimate(IterType beginIter, IterType endIter)
    {
      ...
      brick::numeric::Array2D<Type> S1 =
        brick::numeric::matrixMultiply(D1Transpose, D1);
      brick::numeric::Array2D<Type> S2 =
        brick::numeric::matrixMultiply(D1Transpose, D2);
      brick::numeric::Array2D<Type> S3 =
        brick::numeric::matrixMultiply(D2.transpose(), D2);
      ...
      brick::numeric::Array2D<Type> TT = (
        Type(-1)
        * brick::numeric::matrixMultiply(brick::linearAlgebra::inverse(S3),
                                         S2.transpose()));
      ...
      brick::numeric::Array2D<Type> M0 =
        S1 + brick::numeric::matrixMultiply(S2, TT);
      ...
      brick::numeric::Array2D<Type> eigenvalues;
      brick::numeric::Array2D<Type> eigenvectors;
      ...
      for(unsigned int ii = 0; ii < eigenvalues.size(); ++ii) {
        Type constraintValue =
          4.0 * eigenvectors(0, ii) * eigenvectors(2, ii)
          - eigenvectors(1, ii);
      ...
      for(unsigned int ii = 0; ii < eigenvectors.rows(); ++ii) {
        firstThreeParameters[ii] = eigenvectors(ii, selectedColumn);
      }

      // Solve for the remaining parameters using matrix T, as
      // mentioned above.
      brick::numeric::Array1D<Type> remainingParameters =
        brick::numeric::matrixMultiply(TT, firstThreeParameters);
      ...
      brick::numeric::Vector2D<Type>& semiminorAxis)
    {
      ...
      if(aa != cc) {
        Type aMinusC = aa - cc;
        Type lambda = (brick::common::absoluteValue(aa - cc)
